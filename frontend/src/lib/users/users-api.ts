...
import type { User, UpdateUserPayload, ListUsersParams } from './types';
...
export function deactivateUser(id: string) {
  return apiClient<{ message: string }>(`/users/${id}/deactivate`, { method: 'PATCH' });
}

export function reactivateUser(id: string) {
  return apiClient<{ message: string }>(`/users/${id}/reactivate`, { method: 'PATCH' });
}
