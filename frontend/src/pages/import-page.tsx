...
import {
  IMPORT_TYPE_CONFIG,
  isValidImportType,
} from "@/lib/import/import-type-config";
...
export function ImportPage() {
  ...
  const resolvedType: ImportType | null =
    rawType &&
    isValidImportType(rawType) &&
    (!IMPORT_TYPE_CONFIG[rawType].adminOnly || isAdmin)
      ? rawType
      : isAdmin
        ? null
        : "time-log";
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        description={
          config?.pageDescription ?? "Select an import type to get started."
        }
      />
      {isAdmin && (
        <ImportTypeSelector value={resolvedType} onChange={handleTypeChange} />
      )}
