...
const taskDisplayVariants = cva('border text-foreground', {
  variants: {
    variant: {
      ...
      inline: 'inline-flex items-baseline gap-1.5 rounded-md px-2.5 py-1 text-sm',
...
export function TaskDisplay({ task, variant = 'badge', showDescription, className }: TaskDisplayProps) {
  ...
  const content = (
    <span className={cn(taskDisplayVariants({ variant }), className)}>
      <span className={variant !== 'badge' ? 'font-medium' : undefined}>{task.label}</span>
