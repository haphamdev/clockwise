...
- **New repo write methods must match sibling error handling** — Repository methods that call `prisma.*.create` or `prisma.*.update` often wrap the call in a try/catch for `P2002` (unique constraint). When adding a new write method to an existing repository, check how sibling methods handle Prisma errors and apply the same pattern.
- **String-to-number parsing: don't use truthiness checks** — When converting string data to numbers (CSV imports, query params, form fields), never use `value ? parseFloat(value) : undefined`. The string `'0'` is falsy in JS, so it silently becomes `undefined`. Use `value !== '' ? parseFloat(value) : undefined` instead.
- **`*ForImport` service methods must document skipped validations** — When adding a `createForImport` (or similar) method that bypasses guards present in the regular `create` (e.g., duplicate checks, team validation, active-user checks), add a JSDoc comment listing what's skipped and why (pre-validated by import processor). This prevents future maintainers from assuming the method is unprotected.
