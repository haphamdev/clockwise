...
describe('TasksService', () => {
  ...
  describe('findOrCreate', () => {
    it('should return existing task when label matches', async () => {
      ...
      const result = await service.findOrCreate('project-1', 'JIRA-123', 'user-1');
    ...
    it('should create new task when label does not exist', async () => {
      const newTask = makeTask({ label: 'NEW-TASK', labelNormalized: 'new-task' });
      repo.findByLabel.mockResolvedValue(null);
      repo.create.mockResolvedValue(newTask);

      const result = await service.findOrCreate('project-1', 'NEW-TASK', 'user-1');
  ...
  describe('search', () => {
    it('should validate project access and return results', async () => {
      ...
      const result = await service.search('project-1', 'org-1', 'user-1', false, {
        q: 'JIRA',
        page: 1,
        limit: 10,
      });
    ...
    it('should search without query filter', async () => {
      ...
      await service.search('project-1', 'org-1', 'user-1', true, { page: 1, limit: 10 });
    ...
    it('should propagate PROJECT_NOT_FOUND from validateProjectAccess', async () => {
      const { ProjectNotFoundException } =
        await import('../../common/exceptions/project.exceptions');
      projectsService.validateProjectAccess.mockRejectedValue(new ProjectNotFoundException());

      await expect(
        service.search('bad-project', 'org-1', 'user-1', false, { page: 1, limit: 10 }),
      ).rejects.toThrow(expect.objectContaining({ code: ErrorCode.PROJECT.NOT_FOUND }));
    });
  });
});
