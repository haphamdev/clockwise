...
export class ProjectsService {
  ...
  async findById(
    ...
    if (!isAdmin) {
      const isLinked = await this.projectsRepository.isUserLinkedToProject(
        projectId,
        userId,
      );
      if (!isLinked) {
  ...
  async create(
    ...
    if (!isAdmin) {
      for (const teamId of uniqueTeamIds) {
        const isManager = await this.projectsRepository.isManagerOfTeam(
          teamId,
          userId,
        );
        if (!isManager) {
  ...
  async update(
    ...
    if (!isAdmin) {
      const isManager = await this.projectsRepository.isManagerOfLinkedTeam(
        projectId,
        userId,
      );
      if (!isManager) {
    ...
    const { teamCount, teamIds } =
      await this.projectsRepository.getTeamSummary(projectId);
  ...
  async assignTeam(
    ...
    if (!isAdmin) {
      ...
      const isManagerOfNewTeam = await this.projectsRepository.isManagerOfTeam(
        teamId,
        userId,
      );
      ...
      const isManagerOfLinked =
        await this.projectsRepository.isManagerOfLinkedTeam(projectId, userId);
    ...
    const existing = await this.projectsRepository.findProjectTeam(
      projectId,
      teamId,
    );
    if (existing) {
    ...
    const projectTeam = await this.projectsRepository.assignTeam(
      projectId,
      teamId,
    );
  ...
  async updateSettings(
    ...
    if (
      data.dailyHourLimit === undefined &&
      data.weeklyHourLimit === undefined
    ) {
      const current = await this.projectsRepository.findSettings(projectId);
      // biome-ignore lint/style/noNonNullAssertion: settings always exist for a valid project
      return current!;
    }

    const currentSettings =
      await this.projectsRepository.findSettings(projectId);
    const updated = await this.projectsRepository.updateSettings(
      projectId,
      data,
    );
