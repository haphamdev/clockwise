...
export class InvitationEmailProcessor extends WorkerHost {
  ...
  async process(job: Job<InvitationEmailJobData>): Promise<void> {
    ...
    const claimed = await this.invitationsRepository.updateStatusIf(
      invitationId,
      "initiated",
      "sending",
    );
    if (!claimed) {
      this.logger.warn(
        `Invitation ${invitationId} not in 'initiated' state, skipping`,
      );
      return;
    }
    ...
    if (!invitation || invitation.status !== "sending") {
      this.logger.warn(
        `Invitation ${invitationId} was modified after claiming, skipping`,
      );
      return;
    }
    ...
    const sent = await this.invitationsRepository.updateStatusIf(
      invitationId,
      "sending",
      "sent",
    );
    if (!sent) {
      this.logger.warn(
        `Invitation ${invitationId} status changed during send, not marking as sent`,
      );
    }
  }
  ...
  async onFailed(
    ...
    try {
      await this.invitationsRepository.updateStatusIf(
