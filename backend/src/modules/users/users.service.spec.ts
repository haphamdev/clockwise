...
describe('UsersService', () => {
  ...
  describe('updateUser', () => {
    it('should promote to admin', async () => {
      ...
      const res = await service.updateUser('admin-1', 'user-1', 'org-1', { isAdmin: true });
    ...
    it('should throw LAST_ADMIN when demoting last admin', async () => {
      ...
      await expect(
        service.updateUser('other-admin', 'user-1', 'org-1', { isAdmin: false }),
      ).rejects.toThrow(expect.objectContaining({ code: ErrorCode.USER.LAST_ADMIN }));
    });

    it('should throw CANNOT_MODIFY_SELF when demoting self from admin', async () => {
      repo.findById.mockResolvedValue(makeUser({ id: 'admin-1', isAdmin: true }));
    ...
    it('should replace team assignments after validation', async () => {
      ...
      await service.updateUser('admin-1', 'user-1', 'org-1', { teamAssignments: assignments });
      expect(repo.countValidTeams).toHaveBeenCalledWith('org-1', ['team-2']);
      expect(repo.replaceTeamAssignments).toHaveBeenCalledWith('user-1', assignments);
    });
    ...
    it('should throw INVALID_TEAM_ASSIGNMENT for invalid teamId', async () => {
      ...
      await expect(
        ...
        expect.objectContaining({ code: ErrorCode.USER.INVALID_TEAM_ASSIGNMENT }),
      );
    });
  ...
  describe('deactivateUser', () => {
    ...
    it('should throw CANNOT_MODIFY_SELF', async () => {
      ...
      ).rejects.toThrow(expect.objectContaining({ code: ErrorCode.USER.CANNOT_MODIFY_SELF }));
    });
