...
export class DashboardService {
  ...
  async getMySummary(
    ...
    const workdays = this.buildWorkdays(
      settings.trackSaturday,
      settings.trackSunday,
    );
    ...
    return {
      myHours: {
        ...
        weekOverWeekPct: this.computeChangePercent(
          hours.thisWeek,
          hours.lastWeek,
        ),
        thisMonth: hours.thisMonth,
        lastMonth: hours.lastMonth,
        monthOverMonthPct: this.computeChangePercent(
          hours.thisMonth,
          hours.lastMonth,
        ),
      },
  ...
  async getTeamBreakdown(
    ...
    for (const row of notLogged) {
      ...
      notLoggedByTeam
        .get(row.teamId)
        ?.push({ userId: row.userId, userName: row.userName });
    }

    const breachesByTeam = new Map<
      string,
      { dailyCount: number; weeklyCount: number }
    >();
    for (const row of breaches) {
      breachesByTeam.set(row.teamId, {
    ...
    for (const row of activeProjects) {
      if (!projectsByTeam.has(row.teamId)) {
        projectsByTeam.set(row.teamId, {
          projects: [],
          totalCount: row.totalCount,
        });
      }
      projectsByTeam.get(row.teamId)?.projects.push({
    ...
    const teams = teamHours.map((th) => {
      ...
      return {
        ...
        thresholdBreaches: breachesByTeam.get(th.teamId) ?? {
          dailyCount: 0,
          weeklyCount: 0,
        },
