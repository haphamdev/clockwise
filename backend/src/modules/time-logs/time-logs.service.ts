...
export class TimeLogsService {
  ...
  async create(
    ...
    const listItem = await this.timeLogsRepository.findListItemById(
      timeLog.id,
      orgId,
    );
    ...
    // biome-ignore lint/style/noNonNullAssertion: listItem exists since we just created the time log
  ...
  async createForImport(
    ...
    const task = await this.tasksService.findOrCreate(
      dto.projectId,
      dto.taskLabel,
      targetUserId,
    );
  ...
  async canLogOnBehalf(
    ...
    const targetInOrg = await this.timeLogsRepository.isUserInOrg(
      targetUserId,
      orgId,
    );
    ...
    const managedIds =
      await this.timeLogsRepository.findManagedUserIds(callerUserId);
  ...
  async findAll(
    ...
    if (!isAdmin) {
      const managedIds =
        await this.timeLogsRepository.findManagedUserIds(userId);
      const scope =
        managedIds.length > 0
  ...
  async update(
    ...
    if (after.date !== undefined)
      updateData.date = new Date(dto.date as string);
    ...
    // biome-ignore lint/style/noNonNullAssertion: updated exists since we just modified the time log
  ...
  async computeWarnings(
    ...
    if (projectId) {
      const projectSettings =
        await this.projectsService.getSettingsInternal(projectId);
  ...
  private async assertPermission(
    ...
    const managedUserIds =
      await this.timeLogsRepository.findManagedUserIds(currentUserId);
