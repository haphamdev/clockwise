...
describe('TimeLogsController', () => {
  ...
  describe('create', () => {
    it('should pass userId, orgId, isAdmin and dto to service', async () => {
      ...
      expect(service.create).toHaveBeenCalledWith('user-1', 'org-1', false, dto, undefined);
    ...
    it('should create on behalf when userId differs and permission granted', async () => {
      ...
      expect(service.canLogOnBehalf).toHaveBeenCalledWith('admin-1', true, 'user-2', 'org-1');
      expect(service.create).toHaveBeenCalledWith('user-2', 'org-1', false, dto, 'admin-1');
    });
    ...
    it('should throw CANNOT_LOG_ON_BEHALF when permission denied', async () => {
      ...
      await expect(controller.create(user, dto)).rejects.toThrow(
        expect.objectContaining({ code: ErrorCode.TIME_LOG.CANNOT_LOG_ON_BEHALF }),
      );
    });
  ...
  describe('warnings', () => {
    ...
    it('should compute warnings on behalf when userId provided and permitted', async () => {
      ...
      await controller.warnings(admin, { userId: 'user-2', date: '2026-04-02', hours: 4 });
  ...
  describe('loggableUsers', () => {
    it('should return loggable users from service', async () => {
      ...
      expect(service.getLoggableUsers).toHaveBeenCalledWith('admin-1', 'org-1', true);
    });
  });
});
