...
const EXPECTED_HEADERS = ['date', 'project_name', 'task', 'hours', 'notes', 'user_email'];
...
export class TimeLogImportProcessor implements ImportProcessor {
  ...
  async parseAndValidate(
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      const rowErrors = await this.validateRow(
        data, rowNumber, ctx,
        projectCache, userCache, accessCache, onBehalfCache, duplicateCache,
      );
  ...
  async execute(
    ...
    for (const row of validRows) {
      try {
        ...
        await this.timeLogsService.createForImport(targetUserId, ctx.orgId, ctx.userId, {
          projectId,
          taskLabel,
          date: row.data.date,
          hours,
  ...
  private async validateRow(
    ...
    // Task
    if (!data.task) {
      errors.push({ row: rowNumber, field: 'task', message: 'Task is required' });
    } else if (data.task.length > 100) {
      errors.push({ row: rowNumber, field: 'task', message: 'Task label must be 100 characters or less' });
    }
    ...
    if (userEmail) {
      if (!userCache.has(userEmail)) {
        ...
        userCache.set(userEmail, user ? { id: user.id, orgId: user.orgId, status: user.status } : null);
      }
      const targetUser = userCache.get(userEmail);
      if (!targetUser || targetUser.orgId !== ctx.orgId) {
        errors.push({ row: rowNumber, field: 'user_email', message: `User "${userEmail}" not found in this organization` });
        userResolved = false;
      } else if (targetUser.status !== 'active') {
        errors.push({ row: rowNumber, field: 'user_email', message: `User "${userEmail}" is not active` });
        userResolved = false;
      } else {
        ...
        if (!onBehalfCache.get(cacheKey)) {
          errors.push({ row: rowNumber, field: 'user_email', message: `You do not have permission to log time for "${userEmail}"` });
          userResolved = false;
        }
      }
    } else {
    ...
    if (!projectName) {
      errors.push({ row: rowNumber, field: 'project_name', message: 'Project name is required' });
    } else {
      const cacheKey = projectName.toLowerCase();
      if (!projectCache.has(cacheKey)) {
        const project = await this.projectsService.findActiveByNameInOrg(projectName, ctx.orgId);
        projectCache.set(cacheKey, project ? { id: project.id } : null);
      }
      const project = projectCache.get(cacheKey);
      if (!project) {
        errors.push({ row: rowNumber, field: 'project_name', message: `Project "${projectName}" not found or archived` });
      } else {
        ...
        if (userResolved) {
          ...
          if (!accessCache.has(accessKey)) {
            const hasAccess = await this.projectsService.isUserLinkedToProject(project.id, targetUserId);
            accessCache.set(accessKey, hasAccess);
          }
          if (!accessCache.get(accessKey)) {
            errors.push({ row: rowNumber, field: 'project_name', message: `User does not have access to project "${projectName}"` });
          }
        }
      }
    }
    ...
    if (errors.length === 0 && data._resolved_project_id && data._resolved_user_id && data.task) {
      ...
      if (duplicateCache.get(dupeKey)) {
        errors.push({ row: rowNumber, field: '', message: 'A time log already exists for this user, date, project, and task' });
      } else {
