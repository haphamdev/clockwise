...
describe('InvitationImportProcessor — parseAndValidate', () => {
  ...
  beforeEach(() => {
    ...
    mockInvitationsService.findActiveByEmail = jest.fn().mockResolvedValue(null);
    processor = new InvitationImportProcessor(
      mockInvitationsService, mockTeamsService, mockUsersService,
    );
  });
  ...
  it('should not leak _resolved fields in validRows', async () => {
    const result = await processor.parseAndValidate(validCsv, ctx);
    expect(result.validRows[0].data).not.toHaveProperty('_resolved_team_assignments');
  });
  ...
  it('should include executableRows with resolved team assignments', async () => {
    ...
    const assignments = JSON.parse(result.executableRows[0].data._resolved_team_assignments);
  ...
  describe('email validation', () => {
    ...
    it('should reject invalid email format', async () => {
      ...
      expect(result.validRows).toHaveLength(0);
      expect(result.errors[0]).toMatchObject({
        row: 2, field: 'email', message: expect.stringContaining('valid email'),
      });
    });

    it('should reject email of an already active user', async () => {
      mockUsersService.findByEmail.mockResolvedValue({
        id: 'u-1', orgId, email: 'alice@example.com', status: 'active',
      });
      const result = await processor.parseAndValidate(validCsv, ctx);

      expect(result.validRows).toHaveLength(0);
      expect(result.errors[0]).toMatchObject({
        row: 2, field: 'email', message: expect.stringContaining('already registered'),
      });
    });

    it('should allow email of a pending user', async () => {
      mockUsersService.findByEmail.mockResolvedValue({
        id: 'u-1', orgId, email: 'alice@example.com', status: 'pending',
      });
      const result = await processor.parseAndValidate(validCsv, ctx);
    ...
    it('should allow active user from a different org', async () => {
      mockUsersService.findByEmail.mockResolvedValue({
        id: 'u-1', orgId: 'other-org', email: 'alice@example.com', status: 'active',
      });
      const result = await processor.parseAndValidate(validCsv, ctx);
  ...
  describe('duplicate detection', () => {
    it('should reject email with active invitation', async () => {
      mockInvitationsService.findActiveByEmail.mockResolvedValue({
        id: 'inv-existing', email: 'alice@example.com',
      });
      const result = await processor.parseAndValidate(validCsv, ctx);

      expect(result.validRows).toHaveLength(0);
      expect(result.errors[0]).toMatchObject({
        row: 2, field: 'email', message: expect.stringContaining('active invitation'),
      });
    });
    ...
    it('should detect intra-CSV duplicate emails', async () => {
      ...
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0]).toMatchObject({
        row: 3, message: expect.stringContaining('Duplicate'),
      });
    });
  ...
  describe('teams validation', () => {
    it('should require at least one valid team', async () => {
      ...
      expect(result.validRows).toHaveLength(0);
      expect(result.errors[0]).toMatchObject({
        row: 2, field: 'teams', message: expect.stringContaining('At least one'),
      });
    });
    ...
    it('should error when ALL teams are invalid', async () => {
      ...
      const fatalError = result.errors.find((e: any) => e.message.includes('At least one'));
    ...
    it('should warn and skip unknown manager_teams', async () => {
      ...
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0]).toMatchObject({
        row: 2, field: 'manager_teams', message: expect.stringContaining('Unknown'),
      });
    });
...
describe('InvitationImportProcessor — execute', () => {
  ...
  it('should create invitations and queue emails', async () => {
    ...
    expect(mockInvitationsService.queueInvitationEmail).toHaveBeenCalledWith('inv-new');
  });
  ...
  it('should continue on row failure and report errors', async () => {
    ...
    const rows = [
      { rowNumber: 2, data: { email: 'a@test.com', teams: 'E', manager_teams: '', _resolved_team_assignments: '[{"teamId":"t-1","role":"member"}]' } },
      { rowNumber: 3, data: { email: 'b@test.com', teams: 'E', manager_teams: '', _resolved_team_assignments: '[{"teamId":"t-1","role":"member"}]' } },
    ];
