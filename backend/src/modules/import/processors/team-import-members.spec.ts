    ...
    mockUsersService = {
      findByEmail: jest.fn().mockImplementation((email: string) => {
        if (email === "bob@test.com")
          return Promise.resolve(makeUser("u-bob", "bob@test.com"));
        if (email === "alice@test.com")
          return Promise.resolve(makeUser("u-alice", "alice@test.com"));
  ...
  describe("member resolution", () => {
    ...
    it("should skip unknown manager emails with warning", async () => {
      mockUsersService.findByEmail.mockImplementation((email: string) => {
        if (email === "good@test.com")
          return Promise.resolve(makeUser("u-good", "good@test.com"));
        return Promise.resolve(null);
      });
      const csv =
      ...
      expect(result.errors[0]).toMatchObject({
        field: "managers",
        message: expect.stringContaining("bad@test.com"),
      });
    });
    ...
    it("should allow row with no valid managers (creates team with no members)", async () => {
      ...
      const members = JSON.parse(
        result.executableRows[0].data._resolved_members,
      );
    ...
    it("should skip users from different org", async () => {
      mockUsersService.findByEmail.mockImplementation((email: string) => {
        if (email === "other@test.com")
          return Promise.resolve({
            ...makeUser("u-other", "other@test.com"),
            orgId: "other-org",
          });
        if (email === "bob@test.com")
          return Promise.resolve(makeUser("u-bob", "bob@test.com"));
      ...
      expect(result.errors[0]).toMatchObject({
        field: "members",
        message: expect.stringContaining("other@test.com"),
      });
    });

    it("should skip inactive users", async () => {
      mockUsersService.findByEmail.mockImplementation((email: string) => {
        if (email === "deactivated@test.com")
          return Promise.resolve({
            ...makeUser("u-dead", "deactivated@test.com"),
            status: "deactivated",
          });
        if (email === "bob@test.com")
          return Promise.resolve(makeUser("u-bob", "bob@test.com"));
      ...
      expect(result.errors[0]).toMatchObject({
        field: "members",
        message: expect.stringContaining("deactivated@test.com"),
      });
    });
