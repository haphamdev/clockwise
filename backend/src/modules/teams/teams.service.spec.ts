...
function makeTeamWithMembers(overrides?: Partial<TeamWithMembers>): TeamWithMembers {
  return {
    ...makeTeam(overrides),
    members: [
      {
...
describe('TeamsService', () => {
  ...
  describe('findAll', () => {
    ...
    it('should return only user teams for non-admin', async () => {
      ...
      expect(res).toEqual(result);
      expect(repo.findAllForUser).toHaveBeenCalledWith('org-1', 'user-1', { page: 1, limit: 20 });
    });
  ...
  describe('findById', () => {
    ...
    it('should throw NOT_A_MEMBER for non-member non-admin', async () => {
      ...
      await expect(service.findById('team-1', 'outsider', false)).rejects.toThrow(
  ...
  describe('addMember', () => {
    ...
    it('should throw MEMBER_ALREADY_EXISTS', async () => {
      repo.findEntityById.mockResolvedValue(makeTeam());
      usersService.findById.mockResolvedValue(makeUserWithTeams({ id: 'user-1' }));
